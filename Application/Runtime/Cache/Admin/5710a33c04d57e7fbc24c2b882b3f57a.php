...
<title>角色编辑 - 管理员管理 - 传智播客旅游网站管理系统 v2.0</title>
</head>
<body>
	...
	<form action="" method="post" class="form form-horizontal" id="form-admin-role-add">
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<?php echo ($data["role_name"]); ?>" placeholder="请填写角色名称..." id="role_name" name="role_name">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3">角色描述：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<?php echo ($data["desc"]); ?>" placeholder="请简单的描述角色权限" id="desc" name="desc">
			</div>
		</div>
		<div class="row cl">
				...
				<dl class="permission-list">
					<dd>
						<?php if(is_array($top)): $i = 0; $__LIST__ = $top;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($i % 2 );++$i;?><dl class="cl permission-list2">
							<dt>
								<label class="">
									<input type="checkbox" value="<?php echo ($vol["id"]); ?>" name="auth_id[]" <?php if(in_array(($vol["id"]), is_array($data["auth_id"])?$data["auth_id"]:explode(',',$data["auth_id"]))): ?>checked="checked"<?php endif; ?>/>
							...
							<dd>
								<?php if(is_array($cate)): $i = 0; $__LIST__ = $cate;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 4 );++$i; if($vo["pid"] == $vol["id"] ): ?><label class="">
									<input type="checkbox" value="<?php echo ($vo["id"]); ?>" name="auth_id[]" <?php if(in_array(($vo["id"]), is_array($data["auth_id"])?$data["auth_id"]:explode(',',$data["auth_id"]))): ?>checked="checked"<?php endif; ?>/>
